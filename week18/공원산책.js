function solution(park, routes) {
  ...
  const eastOrWest = (way) => {
    ...
    if (way[0] === "E") {
      if (
        START.col + howManyToGo >= park[0].length ||
        park[START.row]
          .slice(START.col, START.col + howManyToGo + 1)
          .includes("X")
      ) {
        return;
      }
  ...
  const northOrSouth = (way) => {
    ...
    if (way[0] === "N") {
      if (
        START.row - howManyToGo < 0 ||
        park
          .slice(START.row - howManyToGo, START.row)
          .some((row) => row[START.col] === "X")
      ) {
        return;
      }
    ...
    if (way[0] === "S") {
      if (
        START.row + howManyToGo >= park.length ||
        park
          .slice(START.row, START.row + howManyToGo + 1)
          .some((row) => row[START.col] === "X")
      ) {
        return;
      }
